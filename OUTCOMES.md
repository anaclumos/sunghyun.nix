...
| o | Top row: F1/F2/F3 and F7-F12 fire their Apple hardware actions bare (brightness down/up, Mission Control, previous/play-pause/next, mute, volume down/up); F4/F6 send plain F4/F6 bare and F5 sends Control-M bare (owner 2026-08-08; a plain F5 is deliberately no longer reachable); holding fn inverts every one of them (fn+F1 = plain F1 ... fn+F4/F5/F6 = Spotlight/dictation/Do Not Disturb). Built-in Apple keyboard is primary; a non-Apple top row that emits its own consumer usages instead of f1-f12 cannot be expressed this way | nix-darwin `system.defaults.NSGlobalDomain."com.apple.keyboard.fnState" = false` (Apple media base, so nine of the twelve keys need no rule at all) + one Karabiner rule owning f4/f5/f6 (two manipulators per key, bare and fn, so nothing can double-fire), guarded by `variable_unless system.use_fkeys_as_standard_function_keys` so a flipped base state degrades to plain function keys instead of inverting. The preference alone is inert on a running session: IOHIDSystem reads it into `HIDParameters` at login only, and both the macOS driver and Karabiner follow `HIDFKeyMode`, not the plist, so `postActivation` runs `sunghyun fn-state apply` to push it in (`hidutil` cannot, it addresses HID devices and IOHIDSystem is not one) | `defaults read -g com.apple.keyboard.fnState` = 0; `ioreg -c IOHIDSystem` shows `HIDFKeyMode=0` (verify `check_fn_state`); `karabiner_cli --list-system-variables` reports `system.use_fkeys_as_standard_function_keys: false`; the f4/f5/f6 manipulators are present and `karabiner_cli --lint-complex-modifications` passes |
