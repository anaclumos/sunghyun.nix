...
when absent (headless Linux is a first-class target). Nothing in the tree is
compiled: every behaviour is a declaratively generated artifact, so the `sunghyun`
CLI is a `writeShellApplication` assembled from `nix/sunghyun/` with its data
table (apps, IME ids, tile fractions, reserved chords) supplied by
`nix/sunghyun/data.nix`, and the private-framework calls it needs run through
generated JXA rather than a linked binary.
...
| b | Hyper + arrows/1-4/Enter = tiling (left/right/top/bottom, fourths, fullscreen); Hyper+W right three quarters, flush to the right edge, left quarter empty; Hyper+C center, Hyper+V top-left. Every fraction is taken against the visible frame (menu bar and Dock excluded) of the display holding the focused window, so a repeated press is a no-op | Karabiner manipulators → `sunghyun tile <action>` → Hammerspoon over its `hs.ipc` message port. The Accessibility API call happens inside Hammerspoon, so one long-lived process holds the grant instead of one grant per short-lived caller, and `screen:frame()` supplies the visible frame. Fractions and aliases are generated into `~/.hammerspoon/init.lua` from `nix/sunghyun/data.nix`, the same table the CLI validates against | karabiner.json manipulators present; tile actions pass verify (`check_tiles`, `check_tiling_engine`) |
| c | Hyper+J = open system default browser (H/I/K/L/M/N/P/R = mail/IINA/music/calendar/kakaotalk/slack/preview/linear; D = desktop; F = Mission Control). Owner 2026-08-08: I moved from Linear to IINA, and P and R are new. Each one activates a running instance instead of starting a second, and a missing app fails silently rather than raising a dialog on every press | same, `shell_command → /run/current-system/sw/bin/sunghyun open <key>`, resolved through the one generic `open` subcommand against the app table generated from `nix/sunghyun/data.nix` (`open -b <bundle id>`, which activates rather than duplicates) | manipulators present; `sunghyun open-default-browser` resolves; every bound app key resolves (verify `check_apps`, `check_keyboard_engine`) |
...
| l | **Hard invariant: keyboard never bricks.** Any keyboard-layer failure leaves typing working | Engine choice itself: Karabiner-Elements grabber seizes only with a healthy virtual device and its failure mode is "no remap", not "no keys" (2026-08-07 Kanata incident: root exclusive grab + dead VirtualHID output killed every key incl. the Accessibility Keyboard). Kanata remains an alternative engine behind `sunghyun kanata enable --safe` (mechanical passthrough/full proofs + rollback); its LaunchDaemon is flake-default OFF, `sunghyun kanata disable` always works and records a `launchctl disable` override so a plist rename cannot re-arm it. Second failure mode, seen live 2026-08-08: Karabiner 16.1.0's root Core-Service went deaf after three rapid `karabiner.json` relinks while still holding the exclusive grab, which eats every key including the on-screen keyboard. `sunghyun karabiner health` compares the last logged `core_configuration is updated.` against the config symlink's mtime and, only when the daemon missed the relink, restarts the user-domain agent (`launchctl kickstart -k gui/<uid>/org.pqrs.service.agent.Karabiner-Core-Service-rev2`, no sudo) and waits for a fresh re-grab. It runs as the first `post-switch` gate | no root keyboard daemon by default; KE services running; typing unaffected when Input Monitoring is missing; verify `check_keyboard_grab` sees a reload newer than the last relink |
...
| v | Codex CLI (`codex`) and Claude Code CLI (`claude`) present after the one-shot run, on macOS and on screenless Linux alike; interactive zsh ships `cc` → `claude --dangerously-skip-permissions` with arg passthrough (owner 2026-08-08) | macOS: official `codex` and `claude-code` Homebrew casks declared in `homebrew.casks` (vendor release channels, stay writable for self-update). Linux: nixpkgs `codex` and `claude-code` in `nix/home/linux.nix`. Installing is the whole outcome. Sign-in (browser OAuth / API key) stays with the owner's session, same policy as row p, and the retired per-assistant instruction trees stay retired. `cc` is a zsh function in the vendored `assets/dotfiles/zsh/rc/aliases.zsh` (shadows `/usr/bin/cc` in interactive shells only) | verify `check_coding_cli` (`codex`, `claude`); `zsh -ic 'type cc'` reports a function |
...
| aa | Dia is gone from the system (owner 2026-08-08: "Also delete dia from the system"; supersedes the prior install outcome from `8cf7811`) | `thebrowsercompany-dia` dropped from `homebrew.casks`; live uninstall via `brew uninstall --cask thebrowsercompany-dia` (no zap: the cask ships no zap stanza). Aside remains the browser (rows ab, ak) | `/Applications/Dia.app` absent; `brew list --cask` has no `thebrowsercompany-dia` |
| ab | Aside is the system default browser, so Hyper+J opens it (owner 2026-08-08; replaces Dia as the target) | `sunghyun default-browser set`, run as a `post-switch` gate. `-[NSWorkspace setDefaultApplicationAtURL:toOpenURLsWithScheme:completionHandler:]` on the `http` scheme only (macOS derives https and HTML from it and rejects a direct https change). macOS raises its own confirmation panel for this change and has since 10.13; there is no way around it, so the step brings the panel forward and polls, and an unanswered panel is a skip the next switch retries | verify `check_default_browser` (LaunchServices http handler is `at.studio.AsideBrowser`) |
| ac | The Dock holds Finder, the Downloads folder and the Trash, nothing else (owner 2026-08-08: Downloads added to the emptied Dock) | nix-darwin `system.defaults.dock.persistent-apps = [ ]` / `persistent-others = [ { folder = "${home}/Downloads"; } ]` / `show-recents = false`. An empty list rather than the default `null`: nix-darwin drops null options from its `defaults write` list, so null is "unmanaged" and `[ ]` is written as an empty array. The folder entry renders as a `directory-tile` in `persistent-others`, the right-hand Dock section next to the Trash; the submodule's own defaults (sort by name, display as stack, view automatic) are taken as-is since the owner named no view style. Finder and the Trash are Dock fixtures, not preferences, and macOS offers no way to remove them. Activation restarts the Dock itself whenever any dock default is written | verify `check_dock` |
...
| ah | Finder windows show the path bar and the status bar (owner 2026-08-08) | nix-darwin `system.defaults.finder.ShowPathbar = true` and `system.defaults.finder.ShowStatusBar = true`, both typed boolean options at the pinned revision. Activation restarts the Dock after dock writes but never Finder, so a running Finder shows the bars only after its next relaunch; on a fresh machine the desktop-view converge script's own Finder restart covers it, and on an already-converged machine the one switch that introduces the bars needs a one-time Finder restart | verify `check_finder_bars` |
...
| aj | tokenmaxxing (`tokenmaxxing`, alias `xx`) present after the one-shot run, on macOS and on screenless Linux alike (owner 2026-08-08) | flake input `github:anaclumos/tokenmaxxing` (public, ships its own flake; pinned by rev + narHash in `flake.lock`). macOS: `darwinModules.withOverlay` + `programs.tokenmaxxing.enable` in `flake.nix` (systemPackages). Linux: `homeManagerModules.default` in `nix/home/linux.nix` with the input's package set explicitly, because the standalone Home Manager pkgs carries no overlay. Installing the CLI is the whole outcome: `tokenmaxxing init` owns accounts, the `claude` supervisor shim and settings hooks, and stays with the owner (retired shim surfaces stay retired, same policy as row v), and the module's optional check timer keeps its default of off | verify `check_tokenmaxxing` |
| ak | Aside is installed | nix-darwin `homebrew.casks` entry `aside`, the official homebrew/cask token (verified with `brew info --json=v2`, `.tap` = `homebrew/cask`), so no tap trust grant exists to abort a fresh `brew bundle` | verify `check_aside` |
| am | Anything Homebrew-managed that this repo does not declare is absent from the machine; deleting a definition here uninstalls it on the next switch (owner 2026-08-08: "if the definition is gone from this nix repo, it should be gone and uninstalled") | `homebrew.onActivation.cleanup = "uninstall"` (`brew bundle --force-cleanup` against the generated Brewfile, which the flake also publishes at `/etc/sunghyun/Brewfile` so verify can read the declared set). Not `"zap"`: its deeper clean needs Full Disk Access for the process running brew, activation under sudo does not hold FDA, and a failed zap strands a cask half-uninstalled. An eval-time assertion in `nix/darwin/modules/homebrew.nix` fails the build if `karabiner-elements` ever leaves the cask list, because with cleanup on, deleting that line would run its cask uninstall script, which purges the shared DriverKit VirtualHIDDevice files and bricks the keyboard. Exception: `mas` has no uninstall subcommand, so App Store apps (row r) converge on install only and their removal stays manual | verify `check_brew_convergence` (declared casks all installed, no undeclared casks) |
...
3. Accessibility for Hammerspoon (Privacy & Security pane, for window
   placement; the grant belongs to the app bundle, so it survives every rebuild
   and post-switch only opens the pane while it is still missing)
